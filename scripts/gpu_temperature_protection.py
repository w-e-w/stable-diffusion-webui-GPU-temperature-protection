...
shared.options_templates.update(shared.options_section(('GPU_temperature_protection', "GPU Temperature"), {
    ...
    "gpu_temps_sleep_max_sleep_time": shared.OptionInfo(10.0, "Max sleep Time - (during generation)", gr.Number).info("max number of seconds that it's allowed to pause, 0=unlimited"),
    "gpu_temps_sleep_sleep_temp": shared.OptionInfo(75.0, "GPU sleep temperature - (during generation)", gr.Slider, {"minimum": 0, "maximum": 125}).info("generation will pause if GPU core temperature exceeds this temperature"),
    "gpu_temps_sleep_wake_temp": shared.OptionInfo(75.0, "GPU wake temperature - (during generation)", gr.Slider, {"minimum": 0, "maximum": 125}).info("generation will pause until GPU core temperature drops below this temperature"),
    "gpu_temps_sleep_max_sleep_time_setup": shared.OptionInfo(10.0, "Max sleep Time - (before generation starts)", gr.Number).info("max number of seconds that it's allowed to pause, 0=unlimited"),
    "gpu_temps_sleep_sleep_temp_setup": shared.OptionInfo(75.0, "GPU sleep temperature - (before generation starts)", gr.Slider, {"minimum": 0, "maximum": 125}).info("generation will pause if GPU core temperature exceeds this temperature"),
    "gpu_temps_sleep_wake_temp_setup": shared.OptionInfo(75.0, "GPU wake temperature - (before generation starts)", gr.Slider, {"minimum": 0, "maximum": 125}).info("generation will pause until GPU core temperature drops below this temperature"),
}))
...
class GPUTemperatureProtectionScript(scripts.Script):
    ...
    config_setup = TemperatureProtection(
                'gpu_temps_sleep_sleep_temp_setup',
                'gpu_temps_sleep_wake_temp_setup',
                'gpu_temps_sleep_max_sleep_time_setup',
    )

    def setup(self, p, *args):
        if shared.opts.gpu_temps_sleep_enable:
            GPUTemperatureProtectionScript.config_setup.temperature_protection()
